...
describe('DebateDraftPipe', () => {
  ...
  it('throws when debateId is not numeric', () => {
    try {
      ...
      expect((error as HttpException).message).toBe(`'debateId' is expected to be a number.`);
    }
  });
  ...
  it('throws when draftId is not numeric', () => {
    try {
      ...
      expect(error).toBeInstanceOf(HttpException);
      expect((error as HttpException).getStatus()).toBe(HttpStatus.BAD_REQUEST);
      expect((error as HttpException).message).toBe(`'draftId' is expected to be a number.`);
    }
  });
});

...
import {
  getArgumentIdByDebateAndTitle,
  getArgumentSnapshot,
  getDraftArgumentState,
  getSubmittedDraftSnapshot,
...
describe("DebatesService draft flow integration", () => {
  ...
  it("saves a draft without creating a ticket", async () => {
    ...
    const result = await getDraftArgumentState(
      pool,
      debateId,
      "Draft argument",
    );
  ...
  it("returns the saved draft for its owner", async () => {
    const draftId = await getArgumentIdByDebateAndTitle(
      pool,
      debateId,
      "Draft argument",
    );

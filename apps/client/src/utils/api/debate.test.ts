...
describe('fetchDebatesWithFilters', () => {
  ...
  it('maps query → queryStr', async () => {
    ...
    expect(decodeQ(mocked.get.mock.calls[0][0])).toEqual({ queryStr: 'climate' });
  });
  ...
  it('combines all filters', async () => {
    await fetchDebatesWithFilters({ query: 'health', tags: ['7'], tags_match: 'any' });
    expect(decodeQ(mocked.get.mock.calls[0][0])).toEqual({
      queryStr: 'health',
      tags: '7',
      tags_match: 'any',
...
describe('drafts', () => {
  ...
  it('updateDraft PATCHes /debates/:id/draft/:did', async () => {
    await updateDraft({
      debateId: 5,
      draftId: 11,
      draftData: { title: 't', content: 'c', argumentType: 'PRO' as unknown as never },
    });

  ...
  return (
    <div className={styles.debateContent}>
      ...
      <div className={styles.addedBy}>
        Added by: <span>{debateData.username}</span>
      </div>
    </div>
  );
}
...
function UserDebateContent(props: UserDebateContentProps) {
  ...
  const [debateModifiedFields, setDebateModifiedFields] =
    useState<DebateModifiedFields>({ title: undefined });

  const canEditArgument =
    debateData && debateData.boardList !== BoardLists.ACCEPTED;
    ...
    <div className={styles.debateContent}>
      <h1 className={styles.debateTitle}>
        {isEditMode ? (
          <input
            type="text"
            value={debateModifiedFields.title}
      ...
      {debateData.moderatorId ? (
        <div className={styles.reviewedBy}>
          Reviewed by: <span>{debateData.moderatorUsername}</span>
        </div>
      ) : null}
...
function Debate() {
  ...
  const [debate, setDebate] = useState<DebateAsModerator | DebateAsUser | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (user.role === UserRoles.MODERATOR
      ? fetchDebateAsModerator
      : fetchDebateAsUser)(ticketId)
  ...
  const addComment = () => {
    ...
    socket?.emit("comment/debate:create", {
      comment,

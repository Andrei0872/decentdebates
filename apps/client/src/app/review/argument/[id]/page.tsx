...
} from "@/components/RichEditor/plugins/ExportContentPlugin";
...
function ModeratorArgumentContent(props: ModeratorArgumentContentProps) {
  ...
  useEffect(() => {
    ...
    editor?.setEditorState(
      editor.parseEditorState(argumentData.argumentContent),
    );
    ...
    <div className={styles.moderatorArgumentContainer}>
      ...
      <div className={styles.argContainer}>
        ...
        <div className={styles.argInfo}>
          <div
            className={`${styles.argType} ${styles[argumentData.argumentType]}`}
          >
            {argumentData.argumentType}
          </div>

          {argumentData.counterargumentToId ? (
            <div className={styles.counterargumentTitle}>
              Counterargument to{" "}
              <span onClick={props.counterargumentClick}>
                {argumentData.counterargumentToTitle}
              </span>
            </div>
          ) : null}
        </div>
        ...
        <div className={styles.argContent}>
          <RichEditor
            ...
            configOptions={{
              editable: false,
              editorState: argumentData.argumentContent,
            }}
...
function UserArgumentContent(props: UserArgumentContentProps) {
  ...
  const [argumentModifiedFields, setArgumentModifiedFields] =
    useState<ArgumentModifiedFields>({
      argumentTitle: argumentData.argumentTitle,
    });

  const argumentRef = useRef<ExportContentRefData | null>(null);

  const canEditArgument =
    argumentData && argumentData.boardList !== BoardLists.ACCEPTED;
  ...
  const onTitleChanged = (
    ev: BaseSyntheticEvent<InputEvent, any, HTMLInputElement>,
  ) => {
      ...
      <div className={styles.argContainer}>
        <h2 className={styles.argTitle}>
          {isEditMode ? (
            <input
              type="text"
              value={argumentModifiedFields.argumentTitle}
...
function Argument() {
  ...
  >(null);
  const [counterargument, setCounterargument] = useState<DebateArgument | null>(
    null,
  );
  ...
  const canEditArgument =
    argument && argument.boardList !== BoardLists.ACCEPTED;
  ...
  useEffect(() => {
    socket = io(`${process.env.NEXT_PUBLIC_WEBSOCKET_SERVER_URL!}/review`, {
      autoConnect: false,
    ...
    socket.on(
      ...
      (data: { updatedComment: UpdateCommentData }) => {
        const { updatedComment } = data;

        setComments((comments) =>
          comments.map((c) =>
            +c.commentId === +updatedComment.commentId
  ...
  const onCounterargumentClick = () => {
    ...
    fetchArgument(argument.debateId, argument.counterargumentToId).then(
      (counterarg) => setCounterargument(counterarg),
    );
  };
  ...
  return (
    <Layout>
      <div className={styles.panelsContainer}>
        <div
          style={{
            width: shouldDisplayRightPanel
              ? `${windowWidth - RIGHT_PANEL_WIDTH}px`
              : "100%",
          }}
          ...
          <CommentsLayout
            mainContent={
              argument?.reviewItemType === ReviewItemType.MODERATOR ? (
                <ModeratorArgumentContent
                  counterargumentClick={onCounterargumentClick}
                  argumentData={argument}
                />
              ) : argument?.reviewItemType === ReviewItemType.USER ? (
                <UserArgumentContent
                  updateArgument={onUpdateArgument}
                  counterargumentClick={onCounterargumentClick}
                  argumentData={argument}
                />
              ) : (
                  ...
                  <Comment
                    ...
                    renderHeader={() => {
                      ...
                      return (
                        <>
                          <div
                            className={`${styles.commenter} ${user?.id === c.commenterId ? styles.isOwnComment : ""}`}
                          >
                            {c.commenterUsername}
                          </div>
                          ...
                          {c.commenterId === user?.id ? (
                            <div className={styles.commentActionsContainer}>
                              ...
                              >
                                <span>
                                  <Icon
                                    className={styles.commentActionsIcon}
                                    icon="more"
                                  />
                                </span>
                              </Popover>
        ...
        <div
          className={`${styles.rightPanel} ${shouldDisplayRightPanel ? styles.isVisible : ""}`}
        >
          ...
          {shouldDisplayRightPanel && !!counterargument ? (
            <div className={styles.rightPanelContent}>
              <h2 className={styles.counterargumentTitle}>
              ...
              <div className={styles.counterargumentEditorContainer}>
                <RichEditor
                  ...
                  configOptions={{
                    editable: false,
                    editorState: counterargument.content,
                  }}
                />
              </div>
            </div>

...
const mockedFetch = fetchDebatesWithFilters as unknown as ReturnType<
  typeof vi.fn
>;
...
describe("DebatesClient", () => {
  ...
  it('hides the "Start a debate" button when no user is logged in', () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Start a debate" }),
  ...
  it('shows the "Start a debate" button when a user is logged in', () => {
    ...
    expect(
      screen.getByRole("button", { name: "Start a debate" }),
    ).toBeInTheDocument();
  });
  ...
  it("calls fetchDebatesWithFilters with the typed query and swaps the list", async () => {
    ...
    mockedFetch.mockResolvedValueOnce([
      buildDebate(42, { title: "Only match" }),
    ]);

    renderWithStore(
      <DebatesClient debates={[buildDebate(1), buildDebate(2)]} />,
    );

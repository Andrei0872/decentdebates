...
function NewArgumentContent() {
    ...
    useForm<CreateArgumentFormData>({
      defaultValues: {
        ...
        ...(isCounterargumentExplicit && {
          ...
          argType: getCorrespondingCounterargumentType(
            crtDebate?.args.find(
              (a) => +a.argumentId === +counterargumentIdParam!,
            ),
          ),
        }),
      },
  ...
  const [isCounterargumentExpanded, setIsCounterargumentExpanded] = useState(
    () => isCounterargumentExplicit,
  );
  const [counterargument, setCounterargument] = useState<DebateArgument | null>(
    null,
  );
  const [prefilledEditorContent, setPrefilledEditorContent] = useState<
    null | string
  >(null);
  const [isArgumentEditorReady, setIsArgumentEditorReady] = useState(
    () => !isDraft,
  );
  ...
  const onSubmit = (
    formData: CreateArgumentFormData,
    ...
    const createdArgument: CreateArgumentData = {
      ...
      ...(formData.counterargumentId && {
        counterargumentId: +formData.counterargumentId,
      }),
    };
    ...
    action
      .then((res) => {
        ...
        const shouldRedirect =
          isFirstTimeDraft || isDraftSubmitted || isDirectlySubmitted;
        if (!shouldRedirect) {
          return;
        }
      ...
      .catch((err) => {
        ...
        setTimeout(() => {
          toasterRef.current?.show({
            ...
            intent: Intent.NONE,
            message:
              "Please try again and, if the errors persists, please contact support.",
            timeout: 5000,
          });
        }, 1000);
      });
  ...
  const isCounterargument =
    useWatch({ control, name: "isCounterargument" }) === true;
  const watchedCounterargumentId = useWatch({
    control,
    name: "counterargumentId",
  });
  const counterargumentId = isCounterargument
    ? watchedCounterargumentId
    : false;
  ...
  useEffect(() => {
    ...
    if (isCounterargumentExpanded) {
      ...
      fetchArgument(currentDebateId, counterargumentId).then((arg) =>
  ...
  const unexpandedArg = useMemo(() => {
    ...
    return crtDebate?.args.find(
      (arg) => +arg.argumentId === +counterargumentId,
    );
              ...
              >
                <div className={styles.argumentType}>
                  <div className={styles.radioGroup}>
                    <label
                      className={`${styles.argTypeLabel} ${styles.labelPRO}`}
                      htmlFor="pro"
                    >
                      Pro
                    </label>
                  ...
                  <div className={styles.radioGroup}>
                    <label
                      className={`${styles.argTypeLabel} ${styles.labelCON}`}
                      htmlFor="con"
                    >
                      Con
                    </label>
                ...
                <div className={styles.counterargumentCheck}>
                  ...
                  />
                  <div
                    className={`${styles.counterargumentSelect} ${isCounterargument ? "" : styles.isDisabled}`}
                  >
                    <label htmlFor="counterargumentCheck">
                      is counterargument for
                    </label>
                    <select
                      className={styles.counterargumentSelectInput}
                      {...register("counterargumentId")}
                      disabled={!isCounterargument}
                    >
                ...
                {!!counterargumentId ? (
                  <SimpleCollapse
                    expandOnClick={true}
                    header={
                      <p className={styles.counterargumentTitle}>
                        {unexpandedArg?.title}
                      </p>
                    }
                  ...
                  >
                    {counterargument ? (
                      <DebateArgumentCard
                        isExpanded={true}
                        debateArgumentData={counterargument}
                      />
                    ) : (
                      <p>Loading..</p>
                    )}
                ...
                {isArgumentEditorReady ? (
                  <RichEditor
                    ...
                    configOptions={
                      prefilledEditorContent
                        ? { editorState: prefilledEditorContent }
                        : undefined
                    }
                  />
                ) : null}

...
function MyActivity() {
  ...
  useEffect(() => {
    fetchUserActivity().then((activities) => setActivities(activities));
  }, []);
  ...
  const onArgumentClick = (arg: UserActivityArgument) => {
    ...
    router.push(
      `/debates/${arg.debateId}/new-argument?draftId=${arg.argumentId}`,
    );
  };
              ...
              <CardSlider>
                {ongoingItems.map((oi) =>
                  oi.cardType === CardTypes.DEBATE ? (
                    <DebateCard
                      key={oi.ticketId}
                      click={() => onDebateClick(oi)}
                      cardData={oi}
                    />
                  ) : (
                    <ArgumentCard
                      key={oi.ticketId}
                      click={onArgumentClick}
                      cardData={oi}
                    />
                  ),
                )}

...
import type { CurrentDebate, DebateArgument } from '@/store/slices/debates.slice';

const arg = (id: number, counterargumentTo: number | null = null): DebateArgument =>
  ({ argumentId: id, counterargumentTo, counterarguments: null } as unknown as DebateArgument);
...
describe('getDebateDTO', () => {
  ...
  it('attaches a single counterargument', () => {
    ...
    const dto = getDebateDTO(debate);
    expect(dto.args.find(a => a.argumentId === 1)!.counterarguments).toEqual([2]);
    expect(dto.args.find(a => a.argumentId === 2)!.counterarguments).toBeNull();
  });

  it('groups multiple counterarguments under the same parent', () => {
    const debate = build([arg(1), arg(2, 1), arg(3, 1), arg(4, 1)]);
    const dto = getDebateDTO(debate);
    expect(dto.args.find(a => a.argumentId === 1)!.counterarguments).toEqual([2, 3, 4]);
  });
  ...
  it('supports multi-level trees (counters of counters)', () => {
    ...
    const dto = getDebateDTO(debate);
    const byId = Object.fromEntries(dto.args.map(a => [a.argumentId, a.counterarguments]));
    expect(byId).toEqual({
      1: [2, 3],
      2: [4],
      3: null,

...
test.beforeAll(async ({ playwright }) => {
  ...
  const moderatorLoginRes = await moderatorRequest.post(`${API_BASE}/auth/login`, {
    data: { emailOrUsername: MODERATOR_USERNAME, password: MODERATOR_PASSWORD },
  });
  expect(moderatorLoginRes.ok(), `moderator login failed: ${moderatorLoginRes.status()}`).toBeTruthy();
  const moderatorId: number = (await moderatorLoginRes.json()).data.id;

  const activityRes = await moderatorRequest.get(`${API_BASE}/moderator/activity`);
  ...
  expect(pendingCard, 'freshly created debate not found on activity board').toBeTruthy();
  ...
  expect(moveRes.ok(), `moving ticket to IN REVIEW failed: ${moveRes.status()}`).toBeTruthy();
  await moderatorRequest.dispose();

  inReviewDebate = { ...pendingCard!, moderatorId };
});

test('moderator approves an in-review debate and it appears on /debates', async ({ page }) => {
  await page.goto('/');
  ...
  const approvalResponse = page.waitForResponse(r =>
    r.url().includes(`/moderator/approve/debate/${inReviewDebate.ticketId}`) &&
  ...
  await expect(
    page.getByTestId('debate-card').filter({ hasText: inReviewDebate.ticketTitle })
  ).toBeVisible();
});
